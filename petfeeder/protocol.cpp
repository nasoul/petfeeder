...
extern unsigned char mealPlanValue[5];
extern unsigned long manual_feed;
    //此代码为平台自动生成，请按照实际数据修改每个可下发可上报函数和只上报函数
    mcu_dp_raw_update(DPID_MEAL_PLAN,mealPlanValue,5); //RAW型数据上报;
...
static unsigned char dp_download_meal_plan_handle(const unsigned char value[], unsigned short length)
{
    ...
    */
    //Serial.println("meal plan");
    extern SoftwareSerial mySerial;
    mySerial.println(length);
    for(int i=0;i<length;i++)
    {
      mySerial.println(value[i]);  
    }

      extern unsigned char planTime[6];
      planTime[0]=value[0];//week
      planTime[1]=value[1];//h
      planTime[2]=value[2];//m
      planTime[3]=value[3];//v
      planTime[4]=value[4];//e  
    //处理完DP数据后应有反馈
...
static unsigned char dp_download_manual_feed_handle(const unsigned char value[], unsigned short length)
{
    ...
    */
    //Serial.println("manual meal");
    extern Timer timer;
    timer.pulse(10,value[3]*2*1000,LOW);
    //处理完DP数据后应有反馈
...
//    #error "请自行完成相关代码,并删除该行"
    ...
    */
    ...
    if(time[0] == 1) {
        //正确接收到wifi模块返回的格林数据        
        currentTime[1]=time[1];//y
        currentTime[2]=time[2];//m
        currentTime[3]=time[3];//d
        currentTime[4]=time[4]+9;//h
        currentTime[5]=time[5];//m
        currentTime[6]=time[6];//s
//        for(int i =0;i<7;i++)
//          mySerial.println(currentTime[i]);
    }else {
   ...
   */
   ...
   extern SoftwareSerial mySerial;
    if(time[0] == 1) {
        ...
        currentTime[2]=time[2];//m
        currentTime[3]=time[3];//d
        currentTime[4]=time[4];//h
        currentTime[5]=time[5];//m
        currentTime[6]=time[6];//s 
        currentTime[7]=time[7];//w   
        for(int i =0;i<8;i++)
